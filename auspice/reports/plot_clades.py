...
if virus=='H9': ########## H9
    freqs = json.load(open('../data/H9_1y_frequencies.json'))
...
hosts = ['globalh', 'A', 'S', 'H', 'O']
cols = sns.color_palette(n_colors=len(regions))
colsh = sns.color_palette(n_colors=len(hosts))
...
if len(clades):
    ...
    for clade, ax in zip(clades, axs):
        ...
        for c,host in zip(colsh, hosts):
            try:
                tmp_freq = freqs['clades'][host][clade]
                if tmp_freq is not None:
                    ax.plot_date(pivots, tmp_freq,'-o', label = host, c=c)
            except:
                print "skipping", clade, host
...
for mutation, ax in zip(mutations, axs):
    ...
    for c,host in zip(colsh, hosts):
    	try:
    		tmp_freq = freqs['mutations'][host][mutation]
    		if tmp_freq is not None:
    			ax.plot_date(pivots, tmp_freq, '-o',label = host, c=c)
    	except:
    		print "skipping", mutation, host
