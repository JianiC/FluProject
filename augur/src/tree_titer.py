...
class HI_tree(object):
	...
	def save_trunk_cHI(self):
		...
		np.savetxt(self.prefix+self.resolution+'_cHI.txt', cHI_trunk)
		np.savetxt(self.prefix+self.resolution+'_trunk_muts.txt', trunk_muts)

		from random import sample
		n_leafs = 1
		leaf_sample = []
		for y in range(int(self.time_interval[0]), int(self.time_interval[1])):
			tmp_leafs = [leaf for leaf in self.tree.leaf_node_iter() if leaf.num_date>y and leaf.num_date<y+1]
		...
		with open(self.prefix+self.resolution+'_cHI_path.pkl', 'w') as ofile:
...
def get_all_titers_flat(flutype='H9'):
