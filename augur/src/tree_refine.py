...
class tree_refine(object):
	...
	def refine_generic(self, remove_outgroup=True):
		...
		self.node_lookup.update({node.taxon.label.lower():node for node in self.tree.leaf_node_iter()})
		self.node_lookup.update({node.taxon.label.upper():node for node in self.tree.leaf_node_iter()})
