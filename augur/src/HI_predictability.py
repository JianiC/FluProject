...
# It tree and mutation models inferred in intervals of 10years for H9
#
######
...
from H9_process import H9_process, virus_config
...
if mut_models:
    ...
    for res,ax in izip(resolutions,axs):
        ...
        cols = large_effect_mutations(myH9, ax, cols) # plot the mutation trajectories into a multi panel figure
        for mut in myH9.mutation_effects: # for each mutation, make a list of mutation, effect and frequency trajectory
            HI = myH9.mutation_effects[mut]
            mutlabel = mut[0]+':'+mut[1][1:]
            if mutlabel in myH9.frequencies["mutations"]["global"]:
                HI_distributions_mutations.append([res, mut, HI, np.array(myH9.frequencies["mutations"]["global"][mutlabel])])
            else:
...
for node in myH9.tree.postorder_internal_node_iter():
    node.num_date = np.min([c.num_date for c in node.child_nodes()])

assign_fitness(myH9.tree)
dates_fitness = np.array([(n.num_date, n.tol) for n in myH9.tree.postorder_internal_node_iter()])
dates_fitness_term = np.array([(n.num_date, n.tol) for n in myH9.tree.leaf_iter()])

pivots = myH9.tree.seed_node.pivots
HI_vs_max_freq_tree = []
dt=0.5
for node in myH9.tree.postorder_internal_node_iter():
...
for year in range(1990, 2015):
    ...
    myH9.map_HI(training_fraction = 1.0, method = 'nnl1reg', lam_HI=params['lam_HI'], map_to_tree = True,
            lam_pot = params['lam_pot'], lam_avi = params['lam_avi'], cutoff_date = year+2./12.0, subset_strains = False, force_redo = True)

    gof_by_year.append((year, myH9.fit_error, myH9.tree_graph.shape[0]))

    # take and allele frequency snapshot of future season Sept until June
    select_nodes_in_season(myH9.tree, (year+9.0/12, year+18.0/12))
    future_seqs = [node.seq for node in myH9.tree.leaf_iter() if node.alive]
    future_af = allele_freqs(future_seqs)

    #current season May until Feb (all previously selected nodes will be erased)
    select_nodes_in_season(myH9.tree, (year-7.0/12, year+2.0/12))
    af = allele_freqs([node.seq for node in myH9.tree.leaf_iter() if node.alive])
    avg_dist = af_dist(af, future_af)
    max_LBI = calc_LBI(myH9.tree, LBI_tau = 0.001)
    total_alive = 1.0*myH9.tree.seed_node.n_alive
    ...
    for cutoff in cutoffs:
        ...
        nodes = [node for node in myH9.tree.postorder_node_iter()
